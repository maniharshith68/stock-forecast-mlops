...
@patch("src.training.random_forest.mlflow", _mock_mlflow())
@patch("src.training.random_forest.load_tabular_data")
def test_returns_expected_keys(mock_load):
    df = _make_df()
    mock_load.return_value = (df.iloc[:100], df.iloc[100:125], df.iloc[125:])
    with patch("src.training.random_forest.REGISTRY_DIR", Path("/tmp/test_rf")):
        from src.training.random_forest import train_random_forest
        result = train_random_forest("TEST")
